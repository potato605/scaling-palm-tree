// World.h —— 世界：用 unique_ptr 拥有 6 个独立房间
#pragma once
...
public:
    ...
    const std::vector<std::unique_ptr<Room>>& rooms() const { return rooms_; }

    // 跨房间查找（怪物 id 全局唯一）
    Monster* findMonster(const std::string& id);
    bool monsterTreated(const std::string& id);
