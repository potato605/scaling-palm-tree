...
std::string SaveSystem::save(const GameState& st, const World& world, const std::string& path) {
    ...
    {
        std::unordered_map<std::string, std::string> monsterStates = st.monsterStates;
        for (const auto& room : world.rooms()) {
            for (const auto& monster : room->monsters)
                monsterStates[monster->id] = dispositionName(monster->disp);
        }
        os << "monsters=";
        bool first = true;
        for (const auto& [id, disp] : monsterStates) {
            if (!first) os << ",";
            os << id << ":" << disp;
            first = false;
        }
...
// ---------- 解析辅助 ----------
void SaveSystem::parseLine(const std::string& line, std::string& key, std::string& value) {
...
std::string SaveSystem::load(GameState& st, World& world, const std::string& path) {
    ...
    // 兼容压缩地图前的旧存档：原房间 0/1 -> 0，2/3 -> 1，依此类推。
    if (loaded.roomId >= 6) loaded.roomId /= 2;
    loaded.px = geti("px", 1);
    loaded.py = geti("py", 1);
    ...
    if (loaded.checkpointRoom >= 6) loaded.checkpointRoom /= 2;
    ...
    auto parseCsvSet = [](const std::string& s, std::unordered_set<std::string>& out) {
        for (const auto& tok : split(s, ',')) out.insert(tok);
    };
    ...
    if (has("items")) {
        for (const auto& tok : split(kv["items"], ',')) {
            const size_t colon = tok.find(':');
            if (colon == std::string::npos) continue;
            loaded.items[tok.substr(0, colon)] = std::atoi(tok.substr(colon + 1).c_str());
        }
    }
    if (has("monsters")) {
        for (const auto& tok : split(kv["monsters"], ',')) {
            const size_t colon = tok.find(':');
            if (colon == std::string::npos) continue;
            loaded.monsterStates[tok.substr(0, colon)] = tok.substr(colon + 1);
        }
    }
    if (has("final")) {
    ...
    if (loaded.checkpointRoom < 0 || loaded.checkpointRoom >= 6)
        return "存档损坏：检查点异常。";
    ...
    for (auto& r : world.rooms()) {
        for (auto& m : r->monsters) {
            ...
            if (m->treated()) loaded.flags.insert("treated_" + m->id);
        }
    }
