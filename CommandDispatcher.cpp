...
void CommandDispatcher::dispatch(const std::string& rawLine) {
    ...
    switch (st.mode) {
        ...
        case GameMode::PauseOrConfirm:
            ...
            return;
        case GameMode::Title:
            game_.post(L"标题页请按 [1] 新游戏 / [2] 读档 / [3] 退出。", 8);
            return;
        ...
        case GameMode::Combat:
            if (!combatVerb(pc.verb)) {
                game_.post(L"战斗中使用 [1]攻击 [2]净化 [3]控制 [4]使用道具。", 8);
                return;
            }
            break;
    ...
    else if (v == "hint") game_.doHint(pc.args);
