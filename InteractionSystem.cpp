...
std::vector<ContextAction> InteractionSystem::build(GameState& st, World& world,
    ...
    auto monsterTreated = [&](const std::string& id) {
    ...
    for (auto& npc : room->npcs) {
        ...
        if (!done && npc->id == "fangqing" &&
            (!st.hasKey("sample_1") || !monsterTreated("shuang_2"))) {
            a.enabled = false; a.unavailableReason = L"需先完成培育舱目标";
        } else if (!done && npc->id == "chenyan" &&
                   (!st.hasKey("sample_3") || !monsterTreated("haijia_2"))) {
    ...
    for (auto& m : room->monsters) {
        ...
        // 交战
        {
            ...
            if ((m->id == "shuang_1" && !st.hasKey("greenhouse_card")) ||
                (m->id == "lieji_2" && !monsterTreated("lieji_1")) ||
    ...
    for (auto& it : room->interactables) {
        ...
        switch (it.kind) {
            ...
            case Interactable::Kind::GroundItem: {
                ...
                if (a.enabled && ((it.itemId == "sample_1" && !monsterTreated("shuang_2")) ||
                                  (it.itemId == "sample_2" && !monsterTreated("lieji_2")) ||
                                  (it.itemId == "sample_3" && !monsterTreated("haijia_2")))) {
                    a.enabled = false;
