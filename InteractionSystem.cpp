...
std::vector<ContextAction> InteractionSystem::build(const GameState& st, World& world,
    ...
    auto near = [&](int x, int y) {
        return std::abs(x - st.px) + std::abs(y - st.py) <= 1;   // 曼哈顿距离 1
    };
    auto isObj = [&](const std::string& id) {
        return objectiveId && *objectiveId == id;
    };
    const int baseForObj = 10;   // 主线目标最优先

    // ---------- NPC ----------
    for (auto& npc : room->npcs) {
        if (!near(npc->x, npc->y)) continue;
        const bool done = st.hasFlag("talked_" + npc->id);
        ContextAction a;
        a.priority = done ? 90 : (isObj(npc->id) ? baseForObj : 20);
        a.chineseLabel = L"与" + npc->name + L"交谈";
        a.englishLabel = std::wstring(L"talk ") + util::toWide(npc->id);
        a.effectDescription = done ? L"复述当前目标" : L"首次交谈：剧情与生态题（奖励固定获得）";
        a.commandText = "talk " + npc->id;
        a.enabled = !done;
        a.unavailableReason = done ? L"已交谈过" : L"";
        if (!done && npc->id == "fangqing" &&
            (!st.hasKey("sample_1") || !world.monsterTreated("shuang_2"))) {
            a.enabled = false; a.unavailableReason = L"需先完成培育舱目标";
        } else if (!done && npc->id == "chenyan" &&
                   (!st.hasKey("sample_3") || !world.monsterTreated("haijia_2"))) {
            a.enabled = false; a.unavailableReason = L"需先完成围栏区目标";
        } else if (!done && npc->id == "sujin" && !st.hasFlag("talked_chenyan")) {
            a.enabled = false; a.unavailableReason = L"需先完成观察室目标";
        }
        out.push_back(a);
    }
    ...
    for (auto& it : room->interactables) {
        ...
        a.englishLabel = std::wstring(L"open ") + util::toWide(it.id);
    ...
    for (auto& m : room->monsters) {
        ...
        if (m->treated()) {
            ...
            a.englishLabel = std::wstring(L"inspect ") + util::toWide(m->id);
        ...
        // 交战
        {
            ...
            a.englishLabel = std::wstring(L"attack ") + util::toWide(m->id);
            a.effectDescription = m->boss ? L"三阶段战斗 → 最终处置" : L"战斗：攻击/净化/控制";
            a.commandText = "attack " + m->id;
            a.enabled = true;
            if ((m->id == "shuang_1" && !st.hasKey("greenhouse_card")) ||
                (m->id == "lieji_2" && !world.monsterTreated("lieji_1")) ||
                (m->id == "haijia_1" && !st.hasFlag("talked_zhaocheng")) ||
                (m->id == "eps" && !st.hasFlag("talked_sujin"))) {
                a.enabled = false;
                a.unavailableReason = L"需先完成前置任务";
            }
            out.push_back(a);
        }
        if (!m->boss) {
            {
                ...
                a.englishLabel = std::wstring(L"purify ") + util::toWide(m->id);
            ...
            }
            {
                ...
                a.englishLabel = std::wstring(L"control ") + util::toWide(m->id);
    ...
    for (auto& it : room->interactables) {
        ...
        switch (it.kind) {
            ...
            case Interactable::Kind::Terminal: {
                ...
                a.englishLabel = std::wstring(L"access ") + util::toWide(it.id);
            ...
            case Interactable::Kind::Archive: {
                ...
                a.englishLabel = std::wstring(L"read ") + util::toWide(it.id);
            ...
            case Interactable::Kind::Landmark: {
                ...
                a.englishLabel = std::wstring(L"inspect ") + util::toWide(it.id);
            ...
            case Interactable::Kind::GroundItem: {
                ...
                a.englishLabel = std::wstring(L"take ") + util::toWide(it.id);
                ...
                if (a.enabled && ((it.itemId == "sample_1" && !world.monsterTreated("shuang_2")) ||
                                  (it.itemId == "sample_2" && !world.monsterTreated("lieji_2")) ||
                                  (it.itemId == "sample_3" && !world.monsterTreated("haijia_2")))) {
                    a.enabled = false;
                    a.unavailableReason = L"需先处置附近怪物";
                }
                out.push_back(a);
                break;
            }
