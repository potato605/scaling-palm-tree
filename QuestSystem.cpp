...
namespace {
    ...
    const QuestSystem::StageInfo kStages[13] = {
        { L"", L"", 0, L"", L"", L"" },
        { L"进入生态禁区",
          L"在入口安全厅与林阅交谈（地图 N 处，靠近后按数字），回答她的一道基础问题。",
          0, L"talk linyue", L"N",
          L"第一步：入口安全厅。走到 N（林阅）旁边，靠近后按数字交谈。答完题必定获得【温室门禁卡】，补给箱里有生物营养块。" },
        { L"清理温室前廊",
          L"处置温室前廊的双头煞一（靠近触发战斗：攻击到基因衰弱后再净化，或直接肃清）。",
          0, L"attack", L"M",
          L"第二步：从东侧门进入温室前廊。靠近 M（双头煞一）触发战斗：按数字攻击直到它 HP 低于 35%，再选择净化；或继续攻击至 0。避开 ~ 污染水。" },
        { L"清剿诱变培育舱",
          L"处置双头煞二，并取得污染样本一（S）。",
          1, L"take sample_1", L"S",
          L"第三步：诱变培育舱。先靠近 M 战斗处置双头煞二，再按数字拾取 S（污染样本一），顺路可以读取培养日志。" },
        { L"取得实验层权限",
          L"在温室控制室与方晴交谈，取得实验层权限（解锁隔离走廊）。",
          1, L"talk fangqing", L"N",
          L"第四步：温室控制室，与 N（方晴）靠近后按数字交谈答题，获得【实验层权限】，隔离走廊的门自动解锁。" },
        { L"穿越隔离走廊",
        ...
        { L"清剿活体实验舱",
          ...
          2, L"take sample_2", L"S",
          L"第六步：活体实验舱。战斗处置裂脊獾二，拾取 S（样本二），避开 * 碎玻璃区。" },
        { L"取得饲养场钥匙",
          L"在安保值班室与赵诚交谈，取得饲养场钥匙与神经控制协议。",
          3, L"talk zhaocheng", L"N",
          L"第七步：安保值班室。与 N（赵诚）靠近后按数字交谈答题，获得【饲养场钥匙】和【神经控制协议】——控制怪物靠它。" },
        { L"通过饲养场入口",
        ...
        { L"修复破损围栏区",
          L"处置骸甲巨麋二，取得污染样本三（S）。",
          4, L"take sample_3", L"S",
          L"第九步：破损围栏区。处置 M（骸甲巨麋二），拾取 S（样本三）。控制终端可查看状态。" },
        { L"取得中央权限",
          L"在生态观察室与陈砚交谈，取得逆转录净化剂与中央权限。",
          4, L"talk chenyan", L"N",
          L"第十步：生态观察室。与 N（陈砚）靠近后按数字交谈答题，获得【逆转录净化剂】（净化厄生必需）与【中央权限】。" },
        { L"观看苏瑾记录",
          L"在中央控制室前厅观看苏瑾全息影像，了解厄生三阶段与三种处置条件（BOSS 前最后补给）。",
          5, L"talk sujin", L"N",
          L"第十一步：中央控制室前厅。先拾取最终补给（C），再与苏瑾全息影像（N）按数字交谈。" },
        { L"终结厄生",
          L"进入嵌合核心室，击败 APF-X00「厄生」，在最终处置菜单选择：肃清 / 净化 / 控制。",
          5, L"attack eps", L"X",
          L"第十二步：核心室。靠近 X 后按数字攻击，依次打破外壳、暴露核心、令意识崩解，再在最终菜单选择处置方式并确认。" },
    };
}
...
void QuestSystem::checkAndAdvance() {
    ...
    bool advanced = false;
    for (;;) {
        const int s = st.questStage;
        bool done = false;
        switch (s) {
            case 1: done = st.hasFlag("talked_linyue"); break;
            case 2: done = world->monsterTreated("shuang_1"); break;
            case 3: done = world->monsterTreated("shuang_2") && st.hasKey("sample_1"); break;
            case 4: done = st.hasFlag("talked_fangqing"); break;
            case 5: done = world->monsterTreated("lieji_1"); break;
            case 6: done = world->monsterTreated("lieji_2") && st.hasKey("sample_2"); break;
            case 7: done = st.hasFlag("talked_zhaocheng"); break;
            case 8: done = world->monsterTreated("haijia_1"); break;
            case 9: done = world->monsterTreated("haijia_2") && st.hasKey("sample_3"); break;
