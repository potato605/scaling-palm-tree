...
namespace {
    ...
    int smallOffset() {
        std::uniform_int_distribution<int> dist(-2, 2);   // -2..2 小随机偏移
        return dist(rng());
    }
...
Monster* CombatSystem::target() const {
    Room* room = game_.world() ? game_.world()->room(game_.state().roomId) : nullptr;
    return room ? room->monsterById(targetId_) : nullptr;
}
...
void CombatSystem::start(const std::string& monsterId, bool tutorial) {
    ...
    targetId_ = monsterId;
    Monster* m = target();
    if (!m || (!m->alive() && !m->weakened())) return;
...
void CombatSystem::handle(const std::string& verb) {
    if (!active_) return;
    Monster* m = target();
    if (!m || (!m->alive() && !m->weakened())) { finish(); return; }
    ...
    if (verb == "attack") {
        ...
        if (!m->weakened()) {
            ...
            ++unavailableStreak_;
            if (unavailableStreak_ >= 2) pushLog(L"提示：继续攻击怪物，观察它的 HP 条，低于 35% 后按 2 净化。", 14);
            game_.reqRender();
            return;
        }
        applyPurify(*m);
        ...
        if (!m->alive()) {
            pushLog(L"目标已经进入基因衰弱，请直接选择净化或控制。", 8);
            game_.reqRender();
            return;
        }
        guarded_ = true;
        pushLog(L"你进入防御姿态：下一次受到的伤害减半。", 14);
        monsterActs(*m);
        game_.reqRender();
    }
}

void CombatSystem::itemTurn() {
    if (!active_) return;
    Monster* m = target();
    if (!m || !m->alive()) return;
    monsterActs(*m);
...
void CombatSystem::quickPurify(const std::string& monsterId) {
    Room* room = game_.world() ? game_.world()->room(game_.state().roomId) : nullptr;
    Monster* m = room ? room->monsterById(monsterId) : nullptr;
    if (!m || ((!m->alive()) && !m->weakened()) || m->boss) return;
    if (!m->weakened()) {
...
void CombatSystem::checkDisposition(Monster& m) {
    ...
    if (m.hpLow() && m.hp > 0) {
        ...
        game_.state().monsterStates[m.id] = dispositionName(m.disp);
...
void CombatSystem::applyEliminate(Monster& m) {
    ...
    st.setFlag("treated_" + m.id);
    st.monsterStates[m.id] = dispositionName(m.disp);
    ++st.eliminateCount;
    pushLog(m.name + L" 已被肃清。污染通道暂时切断。", 12);
    int ups = Player::addExp(st, m.exp);
    if (ups > 0) pushLog(L"等级提升！Lv." + std::to_wstring(st.level) + L" —— 生命上限、攻击、防御提升。", 11);
...
void CombatSystem::monsterActs(Monster& m) {
    GameState& st = game_.state();
    int dmg = m.attack + (m.boss ? (m.bossPhase - 1) * 3 : 0) - st.defense + smallOffset();
    if (dmg < 1) dmg = 1;
    ...
    if (st.hp * 100 < st.maxHp * 30) {
        pushLog(L"你的生命低于 30%！按 [4] 使用营养块恢复。", 14);
    }
    if (st.hp <= 0) {
        st.hp = 0;
