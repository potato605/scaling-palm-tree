...
void Game::buildSceneTitle() {
    ...
    if (titleTeamOpen_) {
        s.caption = {
            ...
            L"绝世土豆    ·    美味的葱油鸭",
            L"阳爱喝小甜水    ·    幽默果粒橙",
            L"感谢每一位让生态禁区重新运转的研究员。",
        };
        s.buttons = { { 0, L"返回标题", true, L"" } };
    } else {
...
void Game::updateModeTitle() {
    ...
    if (titleTeamOpen_) {
        for (int n = 0; n <= 9; ++n) {
            if (input_.pressed((wchar_t)('0' + n))) {
                titleTeamOpen_ = false;
                render_ = true;
                return;
            }
        }
    ...
    if (input_.pressed('1')) {
        ...
        s.push_back({ ScriptStep::Kind::Lines,
                      { L"（教学）W/A/S/D 移动；靠近目标后按数字交互；Q 退出；F5 存档。" } });
...
void Game::buildSceneNarrative() {
    ...
    s.footer = L"剧情播放中 · Enter 可补全当前行 · 剧情期间不可移动/存读档";
...
void Game::stepCommand(int dir) {
    ...
    if (t.ch == L'L' || t.ch == L'D') {
        ...
        checkTeaching("teach_door", L"（教学）门锁需要对应权限：取得对应物品后，靠近门按数字即可开锁。");
            input_.noteMoveBlocked(dir);
            return;
        }
        // 进入新房间（从西/东 door 落位）
    ...
    if (m2 && m2->alive()) {
        const bool lockedByProgress =
            (m2->id == "shuang_1" && !st_.hasKey("greenhouse_card")) ||
            (m2->id == "lieji_2" && !st_.hasFlag("treated_lieji_1")) ||
            (m2->id == "haijia_1" && !st_.hasFlag("talked_zhaocheng")) ||
            (m2->id == "eps" && !st_.hasFlag("talked_sujin"));
        if (lockedByProgress) {
            post(L"当前目标尚未开放，请先完成任务栏中的前置目标。", 8);
            input_.noteMoveBlocked(dir);
            return;
        }
        post(L"前方是" + m2->name + L"——靠近后按数字开战。", 8);
        input_.noteMoveBlocked(dir);
        return;
    }
    ...
    if (room->tileDangerous(nx, ny)) {
        ...
        if (st_.hp <= 0) {
            st_.hp = 0;
            applyDeath(L"危险地形", L"危险区域会造成真实伤害，请先使用补给并绕开污染水与碎玻璃。" );
        }
        render_ = true;
        return;
    }
...
void Game::onEnterRoom(int roomId) {
    ...
    if (firstVisit) {
        ...
        if (roomId == 1 || roomId == 2 || roomId == 3 || roomId == 5) {
...
void Game::updateModeExploration() {
    ...
    if (infoOpen_) {
        ...
        (void)input_.enterPressed();
        if (any) { infoOpen_ = false; render_ = true; }
        return;
    }

    updateMovement();
    // Enter 不再打开命令行，消费掉边沿避免后续剧情误触发补全。
    (void)input_.enterPressed();
    // 探索模式不再接收命令行字符，丢弃按键采样中的字符缓存，避免跨房间残留。
    input_.takeTyped();
...
void Game::updateModeCommandInput() {
    ...
    if (input_.enterPressed()) {
        ...
        cmdLine_.clear();
        dispatcher_.dispatch(util::toNarrow(line));
...
void Game::buildSceneCombat() {
    ...
    sc.playerLine = L"你：HP " + std::to_wstring(st_.hp) + L"/" + std::to_wstring(st_.maxHp) +
                    L"   攻击 " + std::to_wstring(st_.attack) +
                    L"   防御 " + std::to_wstring(st_.defense);
    sc.hintLine = L"[1]攻击 [2]净化 [3]控制 [4]补给 [5]防御 —— 补给/防御都会消耗一个战斗回合";
    ...
    sc.buttons = {
        ...
        { 5, L"防御", combat_.target() && combat_.target()->alive(),
          combat_.target() && combat_.target()->alive() ? L"" : L"目标已衰弱" },
    };
    // 战斗画面由渲染器统一装配（标题、目标、按钮行均由 renderCombat 负责）
    renderer_.renderCombat(sc);
}
...
void Game::updateModeCombat() {
    ...
    else if (input_.pressed('5')) dispatcher_.dispatch("guard");
    render_ = true;
}
...
void Game::onFinalDisposed(int pick) {
    ...
    st_.monsterStates[boss->id] = dispositionName(boss->disp);
...
void Game::updateModeEnding() {
    if (input_.pressed('0') || input_.pressed('1') || input_.pressed('2') ||
        input_.pressed('3') || input_.pressed('4') || input_.pressed('5') ||
        input_.pressed('6') || input_.pressed('7') || input_.pressed('8') ||
        input_.pressed('9') || input_.pressed('Q') || input_.pressed(VK_RETURN) ||
        input_.pressed(VK_SPACE)) {
        running_ = false;
    }
}
...
void Game::applyDeath(const std::wstring& cause, const std::wstring& advice) {
    ++st_.deaths;
    dieNow({
        L"你因" + cause + L"倒下了。",
        L"死亡不会清空任务与关键物品，你可以从最近检查点继续。",
        advice,
    });
}
...
void Game::doLook(const std::vector<std::string>&) {
    ...
    Room* room = world_->room(st_.roomId);
    if (!room) return;
    post(L"你在" + room->name + L"。使用 W/A/S/D 移动，靠近对象按数字交互。", 7);
...
void Game::doHelp(const std::vector<std::string>&) {
    ...
    infoLines_ = {
        ...
        L"交互：靠近目标后按屏幕上的数字即可完成行动",
        L"战斗：按数字选择攻击、净化、控制、补给或防御",
        L"H 查看当前提示，F5 快速存档，Q 退出游戏",
        L"地图图例位于任务与建议下方，+ 表示已净化，R 表示已控制",
    };
    render_ = true;
}
...
void Game::doInventory(const std::vector<std::string>&) {
    ...
    infoLines_.push_back(L"消耗品：生物营养块 ×" + std::to_wstring(nb) + L"（恢复 55 生命）");
    render_ = true;
}
...
void Game::doStatus(const std::vector<std::string>&) {
    ...
    infoTitle_ = L"角色状态";
    infoLines_ = {
        L"HP " + std::to_wstring(st_.hp) + L"/" + std::to_wstring(st_.maxHp) +
            L"   攻击 " + std::to_wstring(st_.attack) + L"   防御 " + std::to_wstring(st_.defense) +
            L"   经验 " + std::to_wstring(st_.exp),
...
NPC* Game::findNPCInRoom(const std::string& id) {
    Room* room = world_->room(st_.roomId);
    return room ? room->npcById(id) : nullptr;
}
Monster* Game::findMonsterInRoom(const std::string& id) {
    Room* room = world_->room(st_.roomId);
    return room ? room->monsterById(id) : nullptr;
}
Interactable* Game::findInteractableInRoom(const std::string& id) {
    Room* room = world_->room(st_.roomId);
    return room ? room->interactableById(id) : nullptr;
}
...
void Game::doTalk(const std::vector<std::string>& args) {
    if (args.empty()) {
        for (auto& npc : world_->room(st_.roomId)->npcs) {
            if (nearTarget(npc->x, npc->y)) { post(L"靠近" + npc->name + L"后按数字交谈。", 14); return; }
        }
        post(L"附近没有可交谈的人。", 12);
        return;
    }
    ...
    if (npc->id == "fangqing" &&
        (!st_.hasKey("sample_1") || !st_.hasFlag("treated_shuang_2"))) {
        post(L"先完成培育舱的双头煞二与污染样本一目标。", 8);
        return;
    }
    if (npc->id == "chenyan" &&
        (!st_.hasKey("sample_3") || !st_.hasFlag("treated_haijia_2"))) {
        post(L"先完成破损围栏区的骸甲巨麋二与污染样本三目标。", 8);
        return;
    }
    if (npc->id == "sujin" && !st_.hasFlag("talked_chenyan")) {
        post(L"先在生态观察室完成陈砚的任务。", 8);
        return;
    }
...
void Game::startNpcScript(const std::string& npcId) {
    ...
    if (npc->id == "fangqing") { s.push_back({ ScriptStep::Kind::SetCheckpoint, {}, L"", L"", -1, "", 1, st_.px, st_.py }); }
    if (npc->id == "zhaocheng") { s.push_back({ ScriptStep::Kind::SetCheckpoint, {}, L"", L"", -1, "", 3, st_.px, st_.py }); }
    if (npc->id == "chenyan") { s.push_back({ ScriptStep::Kind::SetCheckpoint, {}, L"", L"", -1, "", 4, st_.px, st_.py }); }
    if (npc->id == "sujin") { s.push_back({ ScriptStep::Kind::SetCheckpoint, {}, L"", L"", -1, "", 5, 2, 7 }); }
    ...
    switch (it->kind) {
        ...
        case Interactable::Kind::Container: {
            ...
            if (def) {
                ...
                checkTeaching("teach_item", L"（教学）关键道具与消耗品会显示在屏幕下方的背包栏。");
            }
            hints_.noteProgress(now_);
            render_ = true;
            break;
...
void Game::doInspect(const std::vector<std::string>& args) {
    if (args.empty()) { prelistInteractions(); return; }
    Interactable* it = findInteractableInRoom(args[0]);
    if (!it) {
...
void Game::doTake(const std::vector<std::string>& args) {
    ...
    if ((it->itemId == "sample_1" && !st_.hasFlag("treated_shuang_2")) ||
        (it->itemId == "sample_2" && !st_.hasFlag("treated_lieji_2")) ||
        (it->itemId == "sample_3" && !st_.hasFlag("treated_haijia_2"))) {
        post(L"先完成附近怪物的处置，再拾取污染样本。", 8);
        return;
    }
    it->opened = true;
    const ItemDef* def = findItemDef(it->itemId);
...
void Game::startCombatFromCommand(const std::string& monsterId) {
    ...
    if ((m->id == "shuang_1" && !st_.hasKey("greenhouse_card")) ||
        (m->id == "lieji_2" && !st_.hasFlag("treated_lieji_1")) ||
        (m->id == "haijia_1" && !st_.hasFlag("talked_zhaocheng")) ||
        (m->id == "eps" && !st_.hasFlag("talked_sujin"))) {
        post(L"当前目标尚未开放，请先完成任务栏中的前置目标。", 8);
        return;
    }
        ...
        if (room) {
            for (auto& m : room->monsters) {
                if (m->alive() && nearTarget(m->x, m->y))
                    post(L"可以使用 attack " + util::toWide(m->id) + L"。", 14);
...
void Game::doUse(const std::vector<std::string>&) {
    std::vector<MenuButton> opts;
    int nb = st_.itemCount("nutrient_block");
    const ItemDef* nutrient = findItemDef("nutrient_block");
    const int heal = nutrient ? nutrient->heal : 55;
    opts.push_back({ 1, L"生物营养块 ×" + std::to_wstring(nb) + L"（恢复 " + std::to_wstring(heal) + L"）",
                     nb > 0, nb > 0 ? L"" : L"没有库存" });
    bool inCombat = st_.mode == GameMode::Combat;
    openSelection(L"使用道具", opts, true, [this, heal](int pick) {
        ...
        Player::heal(st_, heal);
        post(L"使用生物营养块：恢复 " + std::to_wstring(heal) + L" 点生命（当前 HP " +
             std::to_wstring(st_.hp) + L"/" + std::to_wstring(st_.maxHp) + L"）。", 10);
        if (!combat_.active()) { st_.mode = GameMode::Exploration; render_ = true; }
        else {
            // 战斗中使用补给占用一个回合，敌人立即行动。
            combat_.itemTurn();
            if (st_.mode != GameMode::GameOver) st_.mode = GameMode::Combat;
            render_ = true;
        }
...
void Game::doSaveImpl() {
    ...
    post(err.empty() ? L"已存档（saves/slot1.txt）。" : L"存档失败：" + util::toWide(err), err.empty() ? 10 : 12);
    render_ = true;
}
...
void Game::doLoadImpl(bool fromTitle) {
    ...
    if (!err.empty()) {
        std::wstring werr = util::toWide(err);
        if (fromTitle) {
            st_ = GameState();
...
void Game::buildSceneRoom() {
    ...
    for (const auto& m : room->monsters) {
        ...
        c.ch = *dispositionGlyph(m->disp);
        if (m->boss && (m->alive() || m->weakened())) c.ch = L'X';
        c.fg = m->boss ? 12 : (m->weakened() ? 13 :
                 m->disp == Monster::Disposition::Purified ? 10 :
                 m->disp == Monster::Disposition::Controlled ? 11 : 7);
    }
    // 5) 主线目标 '!'
    ...
    sc.backpackLine = L"[背包] ";
    bool hasInventory = false;
    for (const ItemDef& def : allItemDefs()) {
        int count = isKeyItemKind(def.kind) ? (st_.hasKey(def.id) ? 1 : 0) : st_.itemCount(def.id);
        if (count <= 0) continue;
        if (hasInventory) sc.backpackLine += L" | ";
        sc.backpackLine += def.name + L" ×" + std::to_wstring(count);
        hasInventory = true;
    }
    if (!hasInventory) sc.backpackLine += L"暂无物品";
    ...
    sc.suggestionLine = L"[建议] " + (quests_.directionText() == L"当前房间"
                                      ...
                                      : L"先前往" + quests_.directionText() + L"，靠近目标后按数字交互。");
    sc.legendLines = {
        L"[解释] @你  N NPC  M怪物  X厄生  I物品  S样本  T终端  C容器",
        L"       D已解锁门  L锁门  +已净化  R已控制  ~污染水  *危险区  !主线目标",
    };
...
void Game::buildSceneEnding() {
    ...
    rows.push_back({ L"生态题正确 " + std::to_wstring(st_.correctAnswers) + L"/5 · 档案 " +
                     std::to_wstring(st_.archivesRead), 7 });
    std::wstring verdict;
    ...
    rows.push_back({ L"", 7 });
    rows.push_back({ L"游戏已经结束，按任意键退出。", 14 });
...
void Game::selftestRun() {
    ...
    check(world_->rooms().size() == 6, "地图已压缩为 6 张");
    bool fixedSize = true;
    for (const auto& room : world_->rooms())
        fixedSize = fixedSize && room->width == 68 && room->height == 15;
    check(fixedSize, "每张地图保持 68×15");
    GameState levelProbe;
    levelProbe.hp = 40;
    levelProbe.exp = Player::expNeedFor(1) - 1;
    Player::addExp(levelProbe, 1);
    check(levelProbe.level == 2 && levelProbe.hp == 40, "升级不会隐性恢复生命");
    ...
    auto fightTo = [&]() {
        ...
        while (combat_.active() && guard++ < 60) {
            ...
            Monster* m = combat_.target();
            if (!m) break;
            if (st_.hp <= st_.maxHp / 3 && st_.itemCount("nutrient_block") > 0) {
                dispatcher_.dispatch("use");
                if (st_.mode == GameMode::Selection) selftestPick(1);
                continue;
            }
            if (st_.hp <= st_.maxHp / 5) {
                dispatcher_.dispatch("guard");
                continue;
            }
    ...
    auto collectAllSupplies = [&]() {
        for (auto& room : world_->rooms()) {
            st_.roomId = room->id;
            for (auto& it : room->interactables) {
                if (it.kind != Interactable::Kind::Container || it.opened || it.itemId != "nutrient_block") continue;
                it.opened = true;
                st_.items[it.itemId] += it.itemCount;
            }
        }
    ...
    collectAllSupplies();
    tp(39, 7);
    startCombat("shuang_1");
    if (scriptActive_) pumpAndScript();
    if (combat_.active()) fightTo();
    check(world_->room(0)->monsterAt(40, 7) && world_->room(0)->monsterAt(40, 7)->treated(), "双头煞一已处置");
    check(st_.hp < st_.maxHp, "战斗结束保留当前伤势，不会自动满血");
    check(world_->room(0)->monsterAt(40, 7) &&
          world_->room(0)->monsterAt(40, 7)->disp == Monster::Disposition::Purified,
          "净化后地图标记为 +");

    // 危险地形应允许生命归零并进入死亡保护，而不是把 HP 永久钳在 1。
    st_.roomId = 2; st_.px = 43; st_.py = 12; st_.hp = 1;
    stepCommand(3);
    check(st_.mode == GameMode::GameOver && st_.hp == 0, "危险地形可触发死亡保护");
    st_.mode = GameMode::Exploration; st_.hp = st_.maxHp; st_.roomId = 0; st_.px = 39; st_.py = 7;

    // ---- 6. 第二张合并地图：双头煞二、样本一、方晴 ----
    check(moveToRoom(1), "进入培育舱与温室控制室");
    tp(15, 7);
    startCombat("shuang_2");
    if (scriptActive_) pumpAndScript();
    if (combat_.active()) fightTo();
    tp(43, 5);
    doTake({ "sample_1" });
    ...
    // ---- 7. 第三张合并地图：两只裂脊獾与样本二 ----
    check(moveToRoom(2), "进入隔离走廊与活体实验舱");
    if (scriptActive_) pumpAndScript();
    tp(35, 7);
    startCombat("lieji_1");
    if (combat_.active()) fightTo();
    ...
    // ---- 8. 第四张合并地图：赵诚、骸甲巨麋一 ----
    check(moveToRoom(3), "进入安保值班室与饲养场入口");
    if (scriptActive_) pumpAndScript();
    tp(13, 7);
    doTalk({ "zhaocheng" });
    pumpAndScript();
    ...
    tp(41, 7);
    startCombat("haijia_1");
    if (combat_.active()) {
        int guard = 0;
        while (combat_.active() && guard++ < 20) {
            Monster* m = combat_.target();
            if (!m) break;
            if (st_.hp <= st_.maxHp / 3 && st_.itemCount("nutrient_block") > 0) {
                dispatcher_.dispatch("use");
            ...
            if (m->weakened()) dispatcher_.dispatch("control");
            else dispatcher_.dispatch("attack");
        }
    }
    check(world_->room(3)->monsterAt(42, 7) &&
          world_->room(3)->monsterAt(42, 7)->disp == Monster::Disposition::Controlled,
          "控制后地图标记为 R");
    // ---- 9. 第五张合并地图：样本三与陈砚 ----
    check(moveToRoom(4), "进入破损围栏区与生态观察室");
    if (scriptActive_) pumpAndScript();
    tp(31, 8);
    startCombat("haijia_2");
    if (combat_.active()) fightTo();
    if (scriptActive_) pumpAndScript();
    tp(52, 10);
    doTake({ "sample_3" });
    ...
    check(moveToRoom(5), "进入中央控制室前厅与嵌合核心室");
    ...
    // ---- 13. 苏瑾 ----
    st_.roomId = 5;   // 清场循环可能把玩家留在其他房间，回到前厅
    tp(33, 7);
    doTalk({ "sujin" });
    pumpAndScript();
    ...
    st_.hp = st_.maxHp;   // 自检从 BOSS 检查点开始，另取最终补给模拟合理策略
    st_.items["nutrient_block"] += 3;
    walkTo(41, 7);
    if (scriptActive_) pumpAndScript();
    if (combat_.active()) fightTo();
