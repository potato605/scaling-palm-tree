...
namespace {
    ...
    const std::string doorOfKey(const std::string& keyId) {
        ...
        if (keyId == "lab_permit") return "g_1_2";
        if (keyId == "farm_key") return "g_2_3";
        if (keyId == "core_permit") return "g_3_4";
        return "";
    }
    // 出题人 NPC id（问卷编号顺序）
...
void Game::buildSceneTitle() {
    ...
    if (titleTeamInfo_) {
        ...
        rows.push_back({ L"", 7 });
        rows.push_back({ L"              ◈ 项 目 成 员 ◈", 11 });
        rows.push_back({ L"", 7 });
        rows.push_back({ L"                绝世土豆", 14 });
        rows.push_back({ L"                美味的葱油鸭", 14 });
        rows.push_back({ L"                阳爱喝小甜水", 14 });
        rows.push_back({ L"                幽默果粒橙", 14 });
        rows.push_back({ L"", 7 });
        rows.push_back({ L"          ——《生态禁区：畸合之源》开发组 ——", 7 });
        rows.push_back({ L"", 7 });
        rows.push_back({ L"              按任意键返回标题", 8 });
...
void Game::updateModeTitle() {
    ...
    if (titleTeamInfo_) {
        // 任意键返回标题
        bool any = input_.pressed(VK_RETURN) || input_.pressed(VK_ESCAPE) ||
                   input_.pressed(VK_SPACE) || input_.pressed('Q') ||
                   input_.pressed('W') || input_.pressed('A') ||
                   input_.pressed('S') || input_.pressed('D') ||
                   input_.pressed('H') || input_.pressed('1') ||
                   input_.pressed('2') || input_.pressed('3') ||
                   input_.pressed('4');
        if (any) { titleTeamInfo_ = false; render_ = true; }
        return;
    }
...
void Game::stepCommand(int dir) {
    ...
    if (t.ch == L'L' || t.ch == L'D') {
        ...
        if (!door->unlocked) {
            ...
            checkTeaching("teach_door", L"（教学）门锁需要对应权限：靠近后按数字尝试开启。补给与权限能开锁。");
            input_.noteMoveBlocked(dir);
            return;
        }
        // 进入新房间（从西/东 door 落位）
    ...
    if (room->tileDangerous(nx, ny)) {
        ...
        st_.hp -= dmg;
        if (st_.hp <= 0) {
            // 修复：HP 掉到 0 应真正死亡（此前被钳制为 1 导致永远不死）
            st_.hp = 0;
            std::vector<std::wstring> notes = {
                L"你踩进了危险区（" + std::wstring(t.ch == L'~' ? L"污染水" : L"碎玻璃") +
                L"），失血过多倒下了。",
                L"死亡不会清空任务与关键物品——已回到最近检查点（生命恢复 60%）。",
            };
            dieNow(notes);
            render_ = true;
            return;
...
void Game::startCombat(const std::string& monsterId) {
    ...
    if (!m || m->treated() || combat_.active()) return;
...
void Game::updateModeEnding() {
    ...
    bool any = input_.pressed(VK_RETURN) || input_.pressed(VK_ESCAPE) ||
               ...
               input_.pressed('W') || input_.pressed('A') ||
               input_.pressed('S') || input_.pressed('D') ||
               input_.pressed(VK_F5) || input_.pressed('H') ||
               input_.pressed('1') || input_.pressed('2') ||
               input_.pressed('3') || input_.pressed('4') ||
               input_.pressed('5') || input_.pressed('6') ||
               input_.pressed('7') || input_.pressed('8') ||
               input_.pressed('9') || input_.pressed('0');
...
void Game::doTalk(const std::vector<std::string>& args) {
    if (args.empty()) {
        for (auto& npc : world_->room(st_.roomId)->npcs) {
            if (nearTarget(npc->x, npc->y)) { post(L"靠近后按数字即可与" + npc->name + L"交谈。", 14); return; }
        }
        post(L"附近没有可交谈的人。", 12);
        return;
    }
...
void Game::startNpcScript(const std::string& npcId) {
    ...
    if (npc->id == "zhaocheng") { s.push_back({ ScriptStep::Kind::SetCheckpoint, {}, L"", L"", -1, "", 2, st_.px, st_.py }); }
    if (npc->id == "chenyan") { s.push_back({ ScriptStep::Kind::SetCheckpoint, {}, L"", L"", -1, "", 3, st_.px, st_.py }); }
    if (npc->id == "sujin") { s.push_back({ ScriptStep::Kind::SetCheckpoint, {}, L"", L"", -1, "", 4, 2, 7 }); }
    ...
    switch (it->kind) {
        ...
        case Interactable::Kind::Container: {
            ...
            if (def) {
                ...
                checkTeaching("teach_item", L"（教学）关键道具与消耗品会显示在状态栏下方的背包栏里。");
            }
            hints_.noteProgress(now_);
            render_ = true;
            break;
...
void Game::startCombatFromCommand(const std::string& monsterId) {
    Monster* m = findMonsterInRoom(monsterId);
    if (!m || m->treated()) { post(L"这里没有可战斗的目标。", 12); return; }
        ...
        if (room) {
            for (auto& m : room->monsters) {
                if (m->alive() && nearTarget(m->x, m->y))
                    post(L"附近有目标：" + m->name + L"，靠近后按数字 1 攻击。", 14);
            }
        }
        post(L"先靠近敌人，进入警戒圈会自动开战。", 8);
        return;
    }
...
void Game::doPurify(const std::vector<std::string>& args) {
    ...
    if (args.empty()) { post(L"先靠近敌人进入战斗，弱化后再按数字净化。", 8); return; }
    Monster* m = findMonsterInRoom(args[0]);
...
void Game::doControl(const std::vector<std::string>& args) {
    ...
    if (args.empty()) { post(L"先靠近敌人进入战斗，弱化后再按数字控制。", 8); return; }
    Monster* m = findMonsterInRoom(args[0]);
    if (!m) { post(L"当前房间没有这个目标。", 12); return; }
    if (m->boss) { post(L"厄生需要在战斗中处置（见最终处置菜单）。", 12); return; }
    if (!nearTarget(m->x, m->y)) { post(L"距离太远。", 12); return; }
...
void Game::buildSceneRoom() {
    ...
    if (auto obj = quests_.objectiveEntity()) {
        ...
        for (const auto& m : room->monsters) {
            if (m->id == *obj && !m->treated()) { FrameCell& c = sc.map.rows[m->y][m->x]; c.ch = L'!'; c.fg = 14; }
        }
    }
    // 6) 玩家
    ...
    // 7) 顶部指令提示 + 彩色状态栏
    sc.topHintLine = L"W/A/S/D 移动 · 数字交互 · Q 退出 · F5 存档";
    sc.hudParts = Player::hudSegments(st_);
    ...
    // 8) 背包栏：消耗品数量 + 关键物品/样本
    {
        ...
        bool first = true;
        int nb = st_.itemCount("nutrient_block");
        if (nb > 0) { bp += L" 生物营养块×" + std::to_wstring(nb); first = false; }
        for (const ItemDef& def : allItemDefs()) {
            if (def.kind == ItemDef::Consumable) continue;
            if (!st_.hasKey(def.id)) continue;
            if (!first) bp += L"  ";
            bp += def.name;
            first = false;
        }
        if (first) bp += L"（空）";
        sc.backpackLines = ConsoleUtil::wrapLine(bp, 74);
    }

    // 9) [任务] / [解释] / [建议]
    sc.objectiveLine = L"[任务] " + quests_.stage().name + L"：" + quests_.stage().next;
    sc.legendLines = {
        L"[解释] @你  N人物  M怪物  X厄生  S样本  I物品",
        L"       C容器  T终端/痕迹  D门(开)  L门(锁)  ~污染水  *危险/肃清  +净化  R控制  !目标",
    };
    sc.suggestionLine = L"[建议] " + (quests_.directionText() == L"当前房间"
                                      ? L"当前房间内行动：先解决目标附近的怪物与物品。"
                                      : L"先前往" + quests_.directionText() + L"，或靠近目标按数字交互。");
...
void Game::selftestRun() {
    // 直接驱动高层 API（不模拟按键），验证核心逻辑：任务链、战斗处置、关键物品、门禁、存档。
    ...
    // 按 id 全局查找怪物处置状态 / 所在房间
    auto treatedOf = [&](const std::string& id) -> bool {
        for (auto& r : world_->rooms())
            for (auto& m : r->monsters)
                if (m->id == id) return m->treated();
        return false;
    };
    auto monsterRoom = [&](const std::string& id) -> int {
        for (auto& r : world_->rooms())
            for (auto& m : r->monsters)
                if (m->id == id) return r->id;
        return -1;
    };
    // 传送到怪物房间并完整处置（教学脚本 → 开战 → 打到净化）
    auto treatMonster = [&](const std::string& id) {
        st_.roomId = monsterRoom(id);
    ...
    // ---- 4. 温室区：双头煞一 / 样本一 / 双头煞二 / 方晴 ----
    treatMonster("shuang_1");
    check(treatedOf("shuang_1"), "双头煞一已处置");
    st_.roomId = 1; tp(51, 9);
    doTake({ "sample_1" });
    check(st_.hasKey("sample_1"), "取得样本一");
    treatMonster("shuang_2");
    check(treatedOf("shuang_2"), "双头煞二已处置");
    st_.roomId = 1; tp(13, 7);
    doTalk({ "fangqing" });
    pumpAndScript();
    check(st_.hasKey("lab_permit"), "答错仍获得实验层权限");
    check(st_.correctAnswers == 1, "答错不增加正确数");

    // ---- 5. 实验层：裂脊獾一 / 裂脊獾二 / 样本二 / 赵诚 ----
    treatMonster("lieji_1");
    check(treatedOf("lieji_1"), "裂脊獾一已处置");
    st_.roomId = 2; tp(51, 6);
    doTake({ "sample_2" });
    check(st_.hasKey("sample_2"), "取得样本二");
    treatMonster("lieji_2");
    check(treatedOf("lieji_2"), "裂脊獾二已处置");
    st_.roomId = 2; tp(13, 7);
    doTalk({ "zhaocheng" });
    pumpAndScript();
    check(st_.hasKey("farm_key") && st_.hasKey("neural_protocol"), "赵诚双件套");

    // ---- 6. 饲养场：骸甲巨麋一 / 骸甲巨麋二 / 样本三 / 陈砚 ----
    treatMonster("haijia_1");
    check(treatedOf("haijia_1"), "骸甲巨麋一已处置");
    st_.roomId = 3; tp(51, 9);
    doTake({ "sample_3" });
    check(st_.hasKey("sample_3"), "取得样本三");
    treatMonster("haijia_2");
    check(treatedOf("haijia_2"), "骸甲巨麋二已处置");
    st_.roomId = 3; tp(13, 7);
    doTalk({ "chenyan" });
    pumpAndScript();
    check(st_.hasKey("retrovirus") && st_.hasKey("core_permit"), "陈砚双件套");

    // ---- 7. 苏瑾 ----
    st_.roomId = 4; tp(33, 7);
    doTalk({ "sujin" });
    pumpAndScript();
    check(st_.hasFlag("talked_sujin"), "苏瑾记录完成");

    // ---- 8. BOSS（三阶段：外壳→核心→意识）----
    st_.roomId = 5; tp(33, 7);
    if (scriptActive_) pumpAndScript();
